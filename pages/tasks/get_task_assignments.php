...
if (!hasAdminPermission('tasks.view')) {
...
try {
    // Get task details
    $stmt = $pdo->prepare("SELECT title, description FROM tasks WHERE id = ? AND admin_created = 1");
    $stmt->execute([$task_id]);
    $task = $stmt->fetch();

    if (!$task) {
        http_response_code(404);
        echo json_encode(['success' => false, 'message' => 'Task not found or not an admin task']);
        exit;
    }
